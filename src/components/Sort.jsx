const Sort = ({
	...
	const selectedOptionName = sort.find(item => item.value === selected).name;
	...
	return (
		<div className="sort">
			...
			>{selectedOptionName}</span></p>
			<div className={selectOpen ? 'sort__select sort__select_open' : 'sort__select'}>
				<ul className="sort__select-inner">
					{sort.map(item => <li
						key={item.id}
						className={`sort__select-option ${item.value === selected ? `sort__select-option_selected ${sortOrder === 'desc' ? 'sort__select-option_desc' : 'sort__select-option_asc'}` : ''}`}
